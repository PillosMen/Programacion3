...
namespace ProyectoX 
{
	...
	public class Funciones : Conexion
	{
		...
		public void EditarRegistro(string id, string nombre, string apellido, string telefono, string direccion){
			this.abrirConexion();
			string sql = " UPDATE clientes SET nombre='" + nombre + "' , apellido='" + apellido + "' , telefono='" + telefono + "' , direccion='" + direccion + "'  WHERE id='" + id + "' ";
			this.ejecutarComando(sql);
			this.cerrarConexion();
		}
